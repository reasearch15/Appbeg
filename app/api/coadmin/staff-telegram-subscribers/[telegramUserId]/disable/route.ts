...
import { disableStaffTelegramSubscriberForCoadmin } from '@/lib/server/staffTelegramSubscribersLedger';
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const subscriber = await disableStaffTelegramSubscriberForCoadmin(
      auth.user.uid,
      telegramUserId
    );
    ...
    console.error('[COADMIN_STAFF_TELEGRAM_SUBSCRIBERS] disable failed', {
    ...
    return NextResponse.json(
      { error: 'Unable to disable Telegram staff right now.' },
      { status: 503 }
    );
  }
}
